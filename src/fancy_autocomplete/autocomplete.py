...
class BaseAutocomplete(object):
    """
    Encapsulates the basic options for doing an autocomplete search for a ``Mosdel```.
    """
    ...
    def __call__(self, request):
        ...
        results = self.get_results_queryset()
...
class ObjectAutocomplete(BaseAutocomplete):
    ...
    def prepare_results(self, results):
        ...
        return list(queryset.values(*response_fields))


class LabelledAutocomplete(BaseAutocomplete):
    """
    ...
    def __init__(self, **kwargs):
        ...
        super(LabelledAutocomplete, self).__init__(kwargs)
    ...
    def prepare_results(self, results):
        ...
        if isinstance(label, basestring):
            ...
            results = results.values_list(key_field, flat=True)
            results = [(result, label(result)) for result in results]
        else:
            raise ImproperlyConfigured("'label' must be either a string or a callable accepting one parameter")
        return results
...
class AutocompleteSite(object):
    ...
    def register(self, key, autocomplete=None, **options):
        ...
        if autocomplete is None:
            autocomplete = LabelledAutocomplete
        self._registry[key] = (autocomplete,  options)
